...
def CourseListView(request, category):
    courses = Lendet.objects.filter(Класс=category)
    context = {
        'courses':courses
    }
...
@login_required
def olusturmak_klase(request):
    if not request.user.profile.is_teacher == True:
        messages.error(request, f'Каттоо эсебиңиз бул URL гана мугалим аккаунттарына кире албайт!')
        return redirect('courses:home')
    if request.method == 'POST':
        form = KlasaForm(data=request.POST, files=request.FILES)
        if form.is_valid():
            form.save()
            messages.success(request, f'Сиздин Курса Түзүлдү.')
            return redirect('courses:home')
    else:
        form = KlasaForm()
    context = {
    ...
    return render(request, 'courses/olusturmak_klase.html', context)


@login_required
def olusturmak_lende(request):
    if not request.user.profile.is_teacher == True:
        messages.error(request, f'Каттоо эсебиңиз бул URL гана мугалим аккаунттарына кире албайт!')
        return redirect('courses:home')
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            Класс = form.cleaned_data['Класс']
            slug = Класс.id
            messages.success(request, f'Сиздин тема түзүлдү.')
    ...
    return render(request, 'courses/olusturmak_lende.html', context)


@login_required
def olusturmak_mesim(request):
    if not request.user.profile.is_teacher == True:
        messages.error(request, f'Каттоо эсебиңиз бул URL гана мугалим аккаунттарына кире албайт!')
        return redirect('courses:home')
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            Туру = form.cleaned_data['Туру']
            slug = Туру.slug
            messages.success(request, f'Сиздин сабагыңыз түзүлдү.')
            return redirect('/courses/' + str(slug) )
    else:
    ...
    return render(request, 'courses/olusturmak_mesim.html', context)
